"""
数据库迁移脚本
用法: python -m migrations.migrate
功能: 自动创建数据库、建表、插入初始数据
"""
import sys
import os
import yaml
        ...
        with conn.cursor() as cur:
            tables = [
                ...
                """CREATE TABLE IF NOT EXISTS `aboutus` (
                    ...
                    `picture1` text, `picture2` text, `picture3` text,
                    PRIMARY KEY (`id`)
...
def seed_data():
    ...
    try:
        with conn.cursor() as cur:
            ...
            # 插入测试用户
            cur.execute(
                "INSERT INTO `yonghu` (`yonghuzhanghao`, `yonghuxingming`, `mima`, `xingbie`, `dianhuahaoma`, `money`) "
                "VALUES ('user1', '张三', '123456', '男', '13800138001', 1000)"
            )
            cur.execute(
                "INSERT INTO `yonghu` (`yonghuzhanghao`, `yonghuxingming`, `mima`, `xingbie`, `dianhuahaoma`, `money`) "
                "VALUES ('user2', '李四', '123456', '女', '13800138002', 500)"
            )
            ...
            for cat in categories:
                cur.execute("INSERT INTO `shujifenlei` (`shujifenlei`) VALUES (%s)", (cat,))
            ...
            cur.execute(
                ...
                ('系统简介', 'SYSTEM INTRODUCTION', '本系统是一个二手书籍在线交易平台，支持用户浏览、购买二手书籍，商家可以发布和管理书籍信息。')
            )

            cur.execute(
                "INSERT INTO `news` (`title`, `introduction`, `picture`, `content`) VALUES (%s, %s, %s, %s)",
                ('欢迎使用二手书籍交易平台', '平台正式上线，欢迎大家使用', 'news_picture1.jpg',
