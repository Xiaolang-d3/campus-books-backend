...
def page():
    ...
    return R_ok(data=FavoriteService.page(request.args.to_dict(), identity))


@storeup_bp.route('/list', methods=['GET'])
def list_all():
    return R_ok(data=FavoriteService.list_all(request.args.to_dict()))


@storeup_bp.route('/info/<int:id>', methods=['GET'])
@login_required_custom
def info(id):
    return R_ok(data=FavoriteService.get_by_id(id))


@storeup_bp.route('/save', methods=['POST'])
@login_required_custom
def save():
    FavoriteService.save(request.json)
    return R_ok()
...
@login_required_custom
def update():
    ok, err = FavoriteService.update(request.json)
...
@login_required_custom
def delete():
    FavoriteService.delete(request.json)
    return R_ok()
