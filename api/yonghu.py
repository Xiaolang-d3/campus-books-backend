...
from services.yonghu_service import UserService
...
def register():
    ok, err = UserService.register(request.json)
...
@login_required_custom
def page():
    return R_ok(data=UserService.page(request.args.to_dict()))


@yonghu_bp.route('/list', methods=['GET'])
def list_all():
    return R_ok(data=UserService.list_all(request.args.to_dict()))


@yonghu_bp.route('/info/<int:id>', methods=['GET'])
@login_required_custom
def info(id):
    return R_ok(data=UserService.get_by_id(id))
...
@login_required_custom
def save():
    ok, err = UserService.save(request.json)
    return R_ok() if ok else R_error(err)
...
@login_required_custom
def update():
    ok, err = UserService.update(request.json)
    return R_ok() if ok else R_error(err)
...
@login_required_custom
def delete():
    UserService.delete(request.json)
    return R_ok()
